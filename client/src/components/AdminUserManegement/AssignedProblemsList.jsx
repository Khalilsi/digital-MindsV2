...
export default function AssignedProblemsList({ assignments = [], onUnassign }) {
  ...
  if (!list.length) {
    return (
      <div className="p-4 bg-white/6 rounded text-white/70">
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold mb-3">Assigned problems</h3>
      ...
      {useVirtual ? (
        ...
        <ul className="space-y-2 max-h-[480px] overflow-auto">
          {list.map((a) => (
            <li
              key={a.id}
              className="flex items-center justify-between bg-white/6 p-3 rounded"
            >
              <div>
